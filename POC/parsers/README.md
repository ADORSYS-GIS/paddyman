...
Parsers must emit a canonical `provenance` mapping on every `NormalizedDocument`
and on bundle outputs. The minimal keys are `path` (absolute path to source)
and `stage` (processing stage). A helper `ensure_provenance()` is available in
`POC/shared/provenance.py` — call it from writers to guarantee consistent keys
and avoid duplicate aliases like `file` vs `path`.
...
### Parser Indices: `openapi_refs`

The writer emits a parser-scoped index `parser_indices.openapi_refs` that maps
`$ref` strings as they appear in the spec to the emitted entity id that
represents the target component. Typical entries look like:

```json
{
  "#/components/schemas/Pet": "<uuid-of-pet-entity>",
  "#/components/schemas/Error": "<uuid-of-error-entity>",
  "#/components/requestBodies/CreatePetRequest": "<uuid-of-requestbody-entity>"
}
```

Consumers should use `parser_indices.openapi_refs[ref]` to locate the
corresponding entity in the bundle's `entities` array. When a `$ref` points to
an external document or an unresolved target, the index may map to the
emitting entity id that referenced it (fallback behavior).
