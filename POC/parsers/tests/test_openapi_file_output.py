...
"""
        ...
        assert len(endpoints) == 2  # POST /payments + GET /payments

        # Verify first endpoint structure and that operation details are
        # represented via separate Operation entities (linked by match_key).
        entity = endpoints[0]
        assert entity["path"] == "/payments"
        assert entity["method"] in ("POST", "GET")
        # Operation-level convenience fields are not present on Endpoint
        assert "operation_id" not in entity
        assert "summary" not in entity
        assert entity.get("api_title") == "Payment API"
        assert "source_file" in entity
        assert "parameters" in entity
        # Ensure an Operation entity exists with the expected operation id
        operations = [e for e in entities if e.get("type") == "Operation"]
        assert any(op.get("properties", {}).get("operation_id") in ("createPayment", "listPayments") for op in operations)
...
"""
        ...
        assert endpoints[0]["path"] == "/v1/{payment-service}/{payment-product}"
