...
def extract_entities_and_relationships(
    ...
    # Normalize entity 'source' to point to the canonical document id
    from shared.provenance import normalize_paths

    for ent in entities:
        old_source = ent.get("source")
        if old_source and old_source != document_id:
        ...
        ent["source"] = document_id

        # Normalize any path-like fields on the entity's provenance/property bag
        props = ent.setdefault("properties", {})
        if "source_metadata" in props and isinstance(props["source_metadata"], dict):
            props["source_metadata"] = normalize_paths(props["source_metadata"]) 
