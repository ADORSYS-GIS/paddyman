...
"""
        ...
        table = doc.source_metadata["metadata"]["tables"][0]
...
"""
        ...
        doc = docs[0]
        assert "tables" in doc.source_metadata["metadata"]
        assert len(doc.source_metadata["metadata"]["tables"]) == 2
        
        assert doc.source_metadata["metadata"]["tables"][0]["headers"] == ["Table1"]
        assert doc.source_metadata["metadata"]["tables"][1]["headers"] == ["Table2", "Col2"]
    ...
    def test_file_without_tables_has_empty_tables_list(self, tmp_path: Path):
        ...
        doc = docs[0]
        assert "tables" in doc.source_metadata["metadata"]
        assert doc.source_metadata["metadata"]["tables"] == []
