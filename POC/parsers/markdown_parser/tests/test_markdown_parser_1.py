...
class TestMarkdownToNormalizedDocs:
    ...
    def test_standard_markdown_file_parsing(self, tmp_path: Path):
        ...
        assert doc.source_metadata["metadata"]["tables"] == []  # No tables in this content
        assert "references" in doc.source_metadata["metadata"]
        assert doc.source_metadata["metadata"]["references"]["inline"] == []  # No references
        assert doc.source_metadata["metadata"]["references"]["definitions"] == []
        assert doc.source_metadata["metadata"]["references"]["citations"] == []
        assert doc.source_metadata["metadata"]["specification"] == "ais_for_deposited_cheques_1_0"
    ...
    def test_file_without_clean_suffix(self, tmp_path: Path):
        ...
        assert doc.source_metadata["metadata"]["specification"] == "specification"
