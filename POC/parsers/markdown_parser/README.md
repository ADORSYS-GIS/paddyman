...
The markdown parser writer emits exactly one normalized JSON bundle per
source Markdown file. Each output filename is derived from the source
file's path relative to `settings.markdown_spec_dir`, sanitized for a
filesystem-safe filename (example: `spec/with/chunk.md` ->
`spec_with_chunk.json`). Each bundle contains a single `documents` entry
whose `source_metadata.location` points to the original source file.
...
Sections and Headings Schema
-----------------------------

The parser now populates a `headings` array and a hierarchical `sections`
array in the document-level metadata. Use these fields to reconstruct the
document outline and to associate extracted tables and lists with their
containing sections.

Example `source_metadata.metadata` snippet:

```json
"headings": [
  {"level": 1, "text": "Title", "line": 1, "id": "title"},
  {"level": 2, "text": "Subheading", "line": 4, "id": "subheading"}
],
"sections": [
  {"title": "Title", "level": 1, "start_line": 1, "end_line": 3, "children": []},
  {"title": "Subheading", "level": 2, "start_line": 4, "end_line": 10, "children": [{"type":"list","index":0},{"type":"table","index":0}]}
]
```

Notes:
- `headings` preserves `level`, `text`, `line`, and a deterministic `id` (slug).
- `sections` provide `start_line`/`end_line` boundaries and `children` references
  to `tables` and `lists` by their index in the respective arrays.
