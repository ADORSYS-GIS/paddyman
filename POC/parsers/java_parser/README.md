...
## Normalized Java Entity Contract

The parser pipeline emits Java entities into the top-level `entities` array of
the normalized parser bundle. Each entry follows the `Entity` shape used by
`shared.models.entity`:

- `type` (required)
- `name` (required)
- `source` (required, references `document_id`)
- `id` (required, stable UUID when possible)
- `properties` (required object)

Java semantic entity types emitted:
...
Common required Java properties in `properties`:
...
- `Class` / `Interface` / `Enum`: `extends`, `implements`, `visibility`, `is_abstract`
- `Method`: `return_type`, `signature`
- `Constructor`: `signature`
- `Field`: `type`, `is_static`, `is_final`, `visibility`
- `Parameter`: `type`, `position`, `is_vararg`, `method_qualified_name`
- `Annotation`: `attributes`, `target_type`, `target_name`
- `Import`: `imported_name`, `is_static`, `is_wildcard`
...
```json
{
  "type": "Class",
  "name": "AspspSettings",
  "source": "java_parser:aspsp-xs2a:aspsp-profile:src/main/java/de/adorsys/psd2/aspsp/profile/domain/AspspSettings.java",
  "id": "2f6a3f3d-b6e8-5d54-9f1b-55c0f9f5d1b1",
  "properties": {
    "fully_qualified_name": "de.adorsys.psd2.aspsp.profile.domain.AspspSettings",
    "package": "de.adorsys.psd2.aspsp.profile.domain",
    "module": "aspsp-profile",
    "repository": "aspsp-xs2a",
    "source_document": "java_parser:aspsp-xs2a:aspsp-profile:src/main/java/de/adorsys/psd2/aspsp/profile/domain/AspspSettings.java",
    "source_path": ".../AspspSettings.java",
...
```json
{
  "type": "Method",
  "name": "probe",
  "source": "java_parser:aspsp-xs2a:aspsp-profile:src/main/java/de/adorsys/psd2/aspsp/profile/domain/ContractProbe.java",
  "id": "b71f4f80-8b7f-5d8d-9c67-2eb8c2d2be81",
  "properties": {
    "fully_qualified_name": "de.adorsys.psd2.aspsp.profile.domain.ContractProbe.probe",
    "package": "de.adorsys.psd2.aspsp.profile.domain",
    "module": "aspsp-profile",
    "repository": "aspsp-xs2a",
    "source_document": "java_parser:aspsp-xs2a:aspsp-profile:src/main/java/de/adorsys/psd2/aspsp/profile/domain/ContractProbe.java",
    "source_path": ".../ContractProbe.java",
    "annotations": [],
    "modifiers": ["public"],
    "return_type": "String",
    "signature": "String de.adorsys.psd2.aspsp.profile.domain.ContractProbe.probe(String value)"
  }
}
```
...
The parser emits a compact top-level `method_calls` index in each module
bundle in addition to explicit `MethodCall` entities. Each entry is a
lightweight record intended for fast lookup by downstream consumers. Fields:

- `id`: stable deterministic id for the call record
- `source`: document id where the call was observed (parser document id)
- `caller_context`: fully-qualified caller identifier (e.g. `pkg.Class#method`)
- `callee_name`: simple method name invoked (e.g. `add`)
- `callee_fqn`: best-effort fully-qualified callee identifier or `null`
- `argument_types`: list of argument type names (best-effort)
- `location`: object with `path` (absolute), `start_line`, `end_line` (1-based)
- `resolved`: boolean indicating whether the callee was resolved to an entity
...
{
  "id": "3b2f...",
  "source": "java_parser:repo:module:src/Example.java",
  "caller_context": "com.example.Foo#process",
  "callee_name": "add",
  "callee_fqn": "java.util.List#add",
  "argument_types": ["String"],
  "location": {"path": "/repo/src/Example.java", "start_line": 42, "end_line": 42},
  "resolved": true
}
```
```
