...
def build_normalized_json(summaries: dict[str, Any] | None = None, parser_output_dir: Path | None = None) -> NormalizedJson:
    ...
    artifacts = collect_parser_bundle_artifacts(parser_output_dir or settings.parser_output_dir)

    return NormalizedJson(
        documents=documents,
        entities=artifacts["entities"],
        relationships=artifacts["relationships"],
        method_calls=artifacts["method_calls"],
        source_metadata=metadata,
        parser_indices=artifacts["parser_indices"],
...
def write_normalized_json(bundle: NormalizedJson, output_path: Path | None = None) -> Path:
    ...
    # Ensure version_metadata follows the project's best-practices contract.
    try:
        vm = getattr(bundle, "version_metadata", None) or {}
        if not isinstance(vm, dict):
            vm = {}
        # Determine sensible default contract based on bundle provenance
        prov = getattr(bundle, "provenance", None) or {}
        stage = prov.get("stage") if isinstance(prov, dict) else None
        parser = prov.get("parser") if isinstance(prov, dict) else None

        default_contract = "parser-json"
        if stage == "parser_pipeline":
            default_contract = "normalized-json"
        elif isinstance(stage, str) and stage in ("java_parser", "openapi_parser", "markdown_parser"):
            default_contract = "parser-json"
        elif isinstance(parser, str) and parser:
            default_contract = "parser-json"

        # Preserve any existing summaries field while ensuring required keys
        summaries = vm.get("summaries") if isinstance(vm, dict) else None
        if not vm.get("contract"):
            vm["contract"] = default_contract
        if not vm.get("version"):
            vm["version"] = "1.0"
        if summaries is not None:
            vm["summaries"] = summaries
        bundle.version_metadata = vm
    except Exception:
        # Never fail writing due to metadata; fall back to a minimal contract
...
def _documents_from_files(root: Path, source_parser: str, patterns: tuple[str, ...]) -> list[NormalizedDocument]:
    ...
    for path in sorted({match for pattern in patterns for match in root.rglob(pattern)}):
        ...
        documents.append(
            NormalizedDocument(
                document_id=make_document_id(source_parser, None, root, path),
                text=text,
