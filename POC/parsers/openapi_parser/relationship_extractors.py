...
def _parameter_schema_relationships(
    parameters: list[dict[str, Any]], schema_index: dict[str, str]
) -> list[dict[str, Any]]:
    """Build USES_SCHEMA relationships from Parameter entities to Schema entities.

    For parameters that reference a schema via `schema_ref`, create a
    `USES_SCHEMA` relationship pointing at the referenced schema entity.
    """
    relationships: list[dict[str, Any]] = []
    for parameter in parameters:
        parameter_id = parameter.get("id")
        schema_ref = parameter.get("schema_ref")
        if not parameter_id or not isinstance(schema_ref, str):
            continue
