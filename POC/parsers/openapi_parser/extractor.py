    ...
    for api_path, path_item in paths.items():
        ...
        for method, operation in path_item.items():
            ...
            if not isinstance(operation, dict):
                continue
            results.append(
                _build_endpoint(str(api_path), method, operation, api_title, spec_source, path_level_params)
            )
...
def _build_endpoint(
    ...
    api_title: str,
    spec_source: str,
    path_level_params: list[dict[str, Any]] | None = None,
) -> EndpointMetadata:
    ...
    # Combine path-level parameters with operation-level parameters (path-level first)
    ...
    if isinstance(combined_params, list):
