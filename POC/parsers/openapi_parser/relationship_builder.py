...
def build_openapi_relationships(entities: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    relationships.extend(_operation_parameter_relationships(operations, param_index))
    ...
    # Link Parameters to Schemas when parameters reference component schemas
    relationships.extend(extractors._parameter_schema_relationships(parameters, schema_index))
    ...
    from openapi_parser.request_body_relationships import operation_request_body_relationships
    # Prefer endpoint-level HAS_REQUEST_BODY links when both endpoint and
    # operation entities reference the same request body. This avoids emitting
    # duplicate HAS_REQUEST_BODY relationships for the same method/path + body.
    endpoint_rb_rels = endpoint_request_body_relationships(endpoints, request_body_index)
    relationships.extend(endpoint_rb_rels)
    op_rb_rels = operation_request_body_relationships(operations, request_body_index)
    # Build set of (target_id, method, path) represented by endpoint relations
    existing_pairs: set[tuple[str | None, str | None, str | None]] = set()
    for r in endpoint_rb_rels:
        tid = r.get("target_entity_id")
        props = r.get("properties") or {}
        m = props.get("endpoint_method")
        p = props.get("endpoint_path")
        existing_pairs.add((tid, m, p))
    # Filter operation-level relations that duplicate endpoint-level ones
    filtered_ops: list[dict[str, Any]] = []
    for r in op_rb_rels:
        tid = r.get("target_entity_id")
        props = r.get("properties") or {}
        m = props.get("operation_method")
        p = props.get("operation_path")
        if (tid, m, p) in existing_pairs:
            continue
        filtered_ops.append(r)
    relationships.extend(filtered_ops)
    ...
    from openapi_parser.response_relationships import operation_response_relationships

    relationships.extend(endpoint_response_relationships(endpoints, response_index))
    relationships.extend(operation_response_relationships(operations, response_index))
...
def _operation_parameter_relationships(
    operations: list[dict[str, Any]], param_index: dict[str, str]
) -> list[dict[str, Any]]:
    """Extract HAS_PARAMETER relationships from operations to parameters."""
    relationships: list[dict[str, Any]] = []
