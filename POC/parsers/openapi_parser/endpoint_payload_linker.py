...
def link_endpoint_payload_entities(
    ...
    _mirror_endpoint_parameters_to_operations(entities)
...
def _link_request_bodies(entities: list[dict[str, Any]], raw: dict[str, Any], spec_source: str) -> None:
    ...
    for link in endpoint_links:
        ...
        if isinstance(endpoint, dict):
            ...
            # Mirror to operation entity when present
            op_key = endpoint.get("operation_match_key")
            if op_key and isinstance(operation_idx, dict):
                operation = operation_idx.get(str(op_key))
                if isinstance(operation, dict):
                    operation["request_body_match_key"] = link.get("request_body_match_key")
                    operation["request_body_required"] = bool(link.get("required", False))
...
def _link_responses(entities: list[dict[str, Any]], raw: dict[str, Any], spec_source: str) -> None:
    ...
    for link in endpoint_links:
        ...
        if op_key and isinstance(operation_idx, dict):
            operation = operation_idx.get(str(op_key))
            if isinstance(operation, dict):
                op_mapping = operation.get("response_match_keys")
                if not isinstance(op_mapping, dict):
                    op_mapping = {}
                    operation["response_match_keys"] = op_mapping
                op_mapping[status_code] = link.get("response_match_key")
...
def _mirror_endpoint_parameters_to_operations(entities: list[dict[str, Any]]) -> None:
    """Copy inline endpoint parameters onto the matching Operation entity.

    Endpoints produced by the extractor may carry inline parameter lists
    (extracted from the operation object). When an Operation entity exists
    for the same (method, path), mirror that list to the Operation so
    operation-level relationship builders can create HAS_PARAMETER
    relationships.
    """
    endpoint_idx = _endpoint_index(entities)
    operation_idx = _operation_index(entities)
    for endpoint in endpoint_idx.values():
        if not isinstance(endpoint, dict):
            continue
        op_key = endpoint.get("operation_match_key")
        if not op_key:
