...
def operation_request_body_relationships(
    operations: list[dict[str, Any]],
    request_body_index: dict[str, str],
) -> list[dict[str, Any]]:
    """Build HAS_REQUEST_BODY relationships from operations to request bodies."""
    relationships: list[dict[str, Any]] = []
    for operation in operations:
        operation_id = operation.get("id")
        match_key = operation.get("request_body_match_key")
        if not operation_id or not isinstance(match_key, str):
            continue
        request_body_id = request_body_index.get(match_key)
        if not request_body_id:
            continue
        ...
        relationships.append({
            ...
            "properties": {
                ...
                "required": bool(operation.get("request_body_required", False)),
            },
            "confidence": 1.0,
        })
    return relationships
