...
# Re-export extraction-loader API from 4_Extractors when present so top-level
# `import loader` resolves consistently across test runs where sys.path
# ordering may expose this package directory before the extractors root.
try:
    import importlib.util, sys
    _candidate = Path(__file__).resolve().parent.parent / "4_Extractors" / "loader.py"
    if _candidate.exists():
        _spec = importlib.util.spec_from_file_location("_extractors_loader_impl", str(_candidate))
    ...
    # Best-effort shim — fall back to graph-normalisation loader behaviour
    pass
